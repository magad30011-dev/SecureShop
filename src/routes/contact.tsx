...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contact the SecureShop team. All form inputs are validated and RASP-protected against injection." },
...
function ContactPage() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    for (const v of [name, email, message]) {
      ...
      if (!r.safe) { setError(`Blocked suspicious input: ${r.threat}`); return; }
    }
    // Defense in depth: escape values before any render/log.
    void escapeHtml(name); void escapeHtml(message);
    setSent(true);
    ...
    <PageShell>
      <section className="bg-gradient-hero py-14 text-white">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6">
          ...
          <p className="mt-3 text-white/80">Questions, feedback, or vulnerability reports — we'd love to hear from you.</p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-1">
          {[
            ...
            <div key={c.label} className="flex items-start gap-3 rounded-xl border border-border bg-gradient-card p-4 shadow-card">
        ...
        <form onSubmit={onSubmit} className="rounded-2xl border border-border bg-gradient-card p-8 shadow-card lg:col-span-2" noValidate>
          ...
          <div className="mt-4">
            ...
            <Textarea id="cmsg" rows={5} value={message} onChange={(e) => setMessage(e.target.value)} maxLength={1000} required />
          ...
          <Button type="submit" className="mt-6 h-11 w-full bg-gradient-primary shadow-glow sm:w-auto">
